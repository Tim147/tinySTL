...
namespace tinySTL {
    ...
    inline ForwardIterator _uninitialed_fill_n(ForwardIterator first, Size n, const T& x, T1*) {
        typedef typename _type_traits<T1>::is_POD_type  _is_POD_type;
        return _uninitial_fill_n_aux(first, n, x, is_POD_type());
    }

    template <class InputIterator, class ForwardIterator>
    inline ForwardIterator
        _uninitialed_copy_aux(InputIterator first, InputIterator end, ForwardIterator result, _true_type) {
        return std::copy(first, end, result); //TODO
    }
    ...
    inline ForwardIterator
        ...
        return std::fill(first, end, x); //TODO
    }
